...
def CCNet(model, ema_model, label_loader, unlabel_loader, test_loader, args):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (unlabel_img, _) in enumerate(tqdm(unlabel_loader)):
            ...
            output_mix = model(input_volume_batch)
            ...
            with torch.no_grad():
                ...
                ema_output_pseudo0 = torch.argmax(torch.softmax(ema_output_ux0, dim=1), dim=1, keepdim=False)
                ema_output_pseudo1 = torch.argmax(torch.softmax(ema_output_ux1, dim=1), dim=1, keepdim=False)

                cutmix_mask = cutmix_mask.squeeze(1)
                batch_pred_mixed = ema_output_pseudo0 * (1.0 - cutmix_mask) + ema_output_pseudo1 * cutmix_mask
                batch_pred_mixed = batch_pred_mixed.long()
            ...
            loss_consistence = med_loss(output_mix[label_bs:], batch_pred_mixed)
            consistency_weight = get_current_consistency_weight(epoch=cur_itrs // 150, args=args)
            loss = loss_sup_mix + consistency_weight * loss_consistence
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            lr_scheduler.step()
            ...
            if cur_itrs % args.step_size == 0:
                mean_dice, mean_hd952, mean_jac, mean_asd = test_acdc(
                    model=model, test_loader=test_loader, args=args, cur_itrs=cur_itrs, name="model1")
                args.logger.info("model1 dice: {:.4f}, hd952: {:.4f},mean_jac: {:.4f},mean_asd: {:.4f}".format(
                    mean_dice, mean_hd952, mean_jac, mean_asd))
                args.writer.add_scalar('mynet/model1_dice', mean_dice, cur_itrs)
                args.writer.add_scalar('mynet/model1_hd95', mean_hd952, cur_itrs)
                args.writer.add_scalar('mynet/model1_Jaccard', mean_jac, cur_itrs)
                args.writer.add_scalar('mynet/model1_ASD', mean_asd, cur_itrs)
                ...
                mean_dice, mean_hd952, mean_jac, mean_asd = test_acdc(
                    model=ema_model, test_loader=test_loader, args=args, cur_itrs=cur_itrs, name="model2")
                args.logger.info("model2 dice: {:.4f}, hd952: {:.4f},mean_jac: {:.4f},mean_asd: {:.4f}".format(
                    mean_dice, mean_hd952, mean_jac, mean_asd))
                args.writer.add_scalar('mynet/model2_dice', mean_dice, cur_itrs)
                args.writer.add_scalar('mynet/model2_hd95', mean_hd952, cur_itrs)
                args.writer.add_scalar('mynet/model2_Jaccard', mean_jac, cur_itrs)
                args.writer.add_scalar('mynet/model2_ASD', mean_asd, cur_itrs)
                ...
                args.logger.info("best model1 dice: {:.4f}, best model1 dice:  {:.4f}".format(best_dice1, best_dice2))
                model.train()
                ema_model.train()
...
def test_acdc(model, test_loader, args, cur_itrs, name="test"):
    ...
    return performance2, mean_hd952, mean_jaccard, mean_asd
...
def calculate_metric_percase(pred, gt):
    ...
    if pred.sum() > 0:
        ...
        jar = metric.jc(pred, gt)
        asd = metric.asd(pred, gt)
        return dice, hd95, jar, asd
    else:
        return 0, 0, 0, 0
