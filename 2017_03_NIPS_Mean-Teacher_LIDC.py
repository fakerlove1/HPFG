...
from val import test_lidc


def main():
    
    path = r"config/mean_teacher_unet_30k_96x96_LIDC.yaml"
...
def Mean_Teacher(model, ema_model, label_loader, unlabel_loader, test_loader, args):
    ...
    args.logger.info("mean_teacher start training")
    for epoch in range(max_epoch):
        ...
        for i, (unlabel_img, _) in enumerate(tqdm(unlabel_loader)):
            ...
            loss_ce = criterion(output[:label_bs], target_label)
            loss_dice = dice_loss(output[:label_bs], target_label.unsqueeze(1), softmax=True)
            loss_sup = 0.4 * loss_ce + 0.6 * loss_dice
            ...
            if cur_itrs > args.total_itrs:
                return
        msg="Train  [{}/{} ({:.0f}%)]\t loss: {:.5f}".format(cur_itrs, args.total_itrs,100. * cur_itrs / args.total_itrs, train_loss / len(unlabel_loader))
