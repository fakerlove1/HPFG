...
class LIDC(Dataset):
    ...
    def load_annotations(self):
        ...
        self.img_dir = [
            self.root + "/image/{}.png".format(item) for item in self.sample_list]
        self.ann_dir = [self.root + "/mask/LIDC_Mask_{}.png".format(
            item.split("_")[1]) for item in self.sample_list]
        self.img_dir = np.array(self.img_dir)
        self.ann_dir = np.array(self.ann_dir)


def get_lidc_loader(root=r'/home/ubuntu/data/LIDC_512x512', batch_size=1, train_crop_size=(96, 96)):
    """
    :param root:
...
def get_ssl_lidc_loader(root=r'/home/ubuntu/data/LIDC/LIDC-DIC', batch_size=8, unlabel_batch_size=24, train_crop_size=(96, 96), label_num=0.2):
    """
    :param root: 数据集路径
    ...
    test_dataset = LIDC(root=root, split="test", transform=test_transform)
    label_loader = DataLoader(train_label, batch_size=batch_size, num_workers=4, shuffle=True, drop_last=False)
