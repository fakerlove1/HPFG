    ...
    for epoch in range(max_epoch):
        ...
        for i, (unlabel_img, _) in enumerate(tqdm(unlabel_loader)):
            ...
            if cur_itrs % args.step_size == 0:
                mean_dice, mean_hd952,mean_jac,mean_asd = test_acdc(model=model, test_loader=test_loader, args=args, cur_itrs=cur_itrs, name="model1")
                args.logger.info("model1 dice: {:.4f}, hd952: {:.4f},mean_jac: {:.4f},mean_asd: {:.4f}".format(mean_dice, mean_hd952,mean_jac,mean_asd))
                ...
                mean_dice, mean_hd952,mean_jac,mean_asd = test_acdc(model=ema_model, test_loader=test_loader, args=args, cur_itrs=cur_itrs, name="model2")
                args.logger.info("model2 dice: {:.4f}, hd952: {:.4f},mean_jac: {:.4f},mean_asd: {:.4f}".format(mean_dice, mean_hd952,mean_jac,mean_asd))
