...
from utils import loadyaml, _get_logger, mk_path,Med_Sup_Loss,DiceLoss
...
def main():
    ...
    path=r"config/unet_30k_96x96_LIDC.yaml"
...
def Supervise(model:torch.nn.Module, train_loader, test_loader, args):
    ...
    for epoch in range(max_epoch):
        for i, (img, label_true) in enumerate(tqdm(train_loader)):
            ...
            loss_ce = criterion(label_pred, label_true)
            loss_dice = dice_loss(label_pred, label_true.unsqueeze(1), softmax=True)
            ...
            if cur_itrs % args.step_size == 0:
                ...
                args.logger.info("epoch:{} \t dice:{:.4f} \t hd95:{:.4f} ".format(epoch, dice, hd95))
