    ...
    try:
        ...
        for epoch in range(max_epoch):
            ...
            for i, (unlabel_img, _) in enumerate(unlabel_loader):
                ...
                # model1 结果
                output_mix, high_feature_mix, head_feature_mix = model1(input_volume_batch)
                output_soft1 = torch.softmax(output_mix, dim=1)

                
                # model2 结果
                ema_output_mix, ema_output_high_feature_mix, ema_output_head_feature_mix = model2(input_volume_batch)
                output_soft2 = torch.softmax(output_mix, dim=1)
                cutmix_mask = cutmix_mask.squeeze(1)
                conf_1, pseudo_outputs1 = torch.max(torch.softmax(ema_output_mix[label_bs:].detach(), dim=1), dim=1)
                # pseudo_outputs1[conf_1 < 0.5] = 255

                pseudo_pred_mixed = target_label1 * (1.0 - cutmix_mask) + pseudo_outputs1 * cutmix_mask
                pseudo_pred_mixed = pseudo_pred_mixed.long()

                loss_1 = med_loss(output_mix[:label_bs], target_label)
                loss_2 = med_loss(ema_output_mix[:label_bs], target_label)

                loss_sup = loss_1+loss_2

                loss_contrastive1 = dense_loss(high_feature_mix, ema_output_high_feature_mix)
                loss_contrastive2 = dense_loss(ema_output_high_feature_mix, high_feature_mix)

                loss_consistence = torch.mean((output_soft1[label_bs:] - ema_output_mix[label_bs:].detach()) ** 2)
                consistency_weight = get_current_consistency_weight(epoch=cur_itrs//150, args=args)

                pseudo_outputs1 = torch.argmax(output_soft1[label_bs:].detach(), dim=1, keepdim=False)
                pseudo_outputs2 = torch.argmax(output_soft2[label_bs:].detach(), dim=1, keepdim=False)

                pseudo_supervision1 = dice_loss(output_soft1[label_bs:], pseudo_outputs2.unsqueeze(1))
                pseudo_supervision2 = dice_loss(output_soft2[label_bs:], pseudo_outputs1.unsqueeze(1))

                loss_semi = consistency_weight * (pseudo_supervision1+pseudo_supervision2 + loss_contrastive1+loss_contrastive2)
