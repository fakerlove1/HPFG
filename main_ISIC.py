...
from utils import build_lr_scheduler, build_optimizer,Dense_Loss,BoxMaskGenerator
...
def main():
    ...
    path = r"config/ccnet_unet_30k_224x224_ISIC.yaml"
...
class Med_Sup_Loss(nn.Module):
    ...
    def forward(self, outputs, target_label):
        loss = 0.4 * self.ce_loss(outputs, target_label) + 0.6 * self.dice_loss(torch.softmax(outputs, dim=1), target_label.unsqueeze(1))
        return loss
    ...
    for epoch in range(max_epoch):
        ...
        for i, (unlabel_img, _) in enumerate(tqdm(unlabel_loader)):
            ...
            with torch.no_grad():
                ...
                ema_output_ux0=torch.argmax(torch.softmax(ema_output_ux0, dim=1), dim=1, keepdim=False)
                ema_output_ux1=torch.argmax(torch.softmax(ema_output_ux1, dim=1), dim=1, keepdim=False)

                cutmix_mask=cutmix_mask.squeeze(1)
                batch_pred_mixed=ema_output_ux0*(1.0-cutmix_mask)+ema_output_ux1*cutmix_mask
            ...
            loss_contrastive = dense_loss(high_feature_mix, ema_output_high_feature_mix) + dense_loss(head_feature_mix, ema_output_head_feature_mix)
