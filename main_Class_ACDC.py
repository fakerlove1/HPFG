...
from utils import build_lr_scheduler, build_optimizer, Med_Sup_Loss,Pixel_Class_Contrastive_Loss
...
def CCNet(model, ema_model, label_loader, unlabel_loader, test_loader, args):
    ...
    class_loss = Pixel_Class_Contrastive_Loss(args.num_classes)
    ...
    for epoch in range(max_epoch):
        ...
        for i, (unlabel_img, _) in enumerate(tqdm(unlabel_loader)):
            ...
            loss_contrastive=class_loss(head_feature_mix[1][:label_bs],ema_output_mix[1][:label_bs],target_label,output_mix[:label_bs],ema_output_mix[:label_bs])
