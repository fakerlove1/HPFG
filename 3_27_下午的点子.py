        ...
        for epoch in range(max_epoch):
            ...
            for i, (unlabel_img, _) in enumerate(unlabel_loader):
                ...
                with torch.no_grad():
                    ...
                    # ema_output_ux0, ema_output_high_feature0, ema_output_head_feature0 = ema_model(label_img1)
                    # ema_output_ux1, ema_output_high_feature1, ema_output_head_feature1 = ema_model(unlabel_img)
                ...
                loss_u_ce = ce_loss(output_mix[label_bs:], pseudo_pred_mixed)
                # loss_u_dice = dice_loss(output_soft1[label_bs:], pseudo_pred_mixed.unsqueeze(1))
                # loss_pseudo = 0.5 * (loss_u_ce + loss_u_dice)

                loss = loss_sup_mix + 0.5 * loss_contrastive + 0.5 * loss_u_ce
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                lr_scheduler.step()
