...
class ChannelTransformer(nn.Module):
    def __init__(self, vis, img_size, channel_num=[64, 128, 256, 512], patchSize=[32, 16, 8, 4]):
        ...
        self.embeddings_2 = Channel_Embeddings(self.patchSize_2, img_size=img_size // 2,
                                               in_channels=channel_num[1])
        self.embeddings_3 = Channel_Embeddings(self.patchSize_3, img_size=img_size // 4,
                                               in_channels=channel_num[2])
        self.embeddings_4 = Channel_Embeddings(self.patchSize_4, img_size=img_size // 8,
...
class UCTransNet(nn.Module):
    def __init__(self, n_classes, n_channels=3, deep_supervision=True, **kwargs):
        super().__init__()
...
if __name__ == '__main__':
    X = torch.Tensor(16, 3, 96, 96)
    model = UCTransNet(1)
    # print(count_params(model)/1000/1000)  #66
    out = model(X)
    # print(out.shape)
