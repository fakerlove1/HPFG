...
class EncoderBottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, base_width=64):
        ...
        self.conv2 = nn.Conv2d(width, width, kernel_size=3,
                               stride=2, groups=1, padding=1, dilation=1, bias=False)
...
class TransUNet_LIDC(nn.Module):
    def __init__(self,image_size=96,num_classes=4,in_channels=3,out_channels=128,head_num=4,mlp_dim=512,block_num=8,patch_dim=16):
        super().__init__()

        self.encoder = Encoder(image_size, in_channels, out_channels,head_num, mlp_dim, block_num, patch_dim)
...
def get_transunet_LIDC(image_size=96,
                  num_classes=4,
                  in_channels=3):
    if isinstance(image_size, list) or isinstance(image_size, tuple):
        image_size = image_size[0]
    return TransUNet_LIDC(image_size, num_classes, in_channels)
...
if __name__ == '__main__':
    ...
    model = get_transunet_LIDC(image_size=96)
    y = model(x)
    print(y.shape)

    # print(sum(p.numel() for p in transunet.parameters()))
    # print(transunet(torch.randn(2, 3, 224, 224)).shape)
