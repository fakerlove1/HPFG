...
def main():
    path = r"config/ict-medseg_unet_30k_224x224_Synapse.yaml"
    ...
    label_loader, unlabl_loader, test_loader, = build_loader(args)  # 构建数据集
    args.epochs = args.total_itrs // args.step_size  # 设置模型epoch
    args.logger.info("==========> label_loader length:{}".format(len(label_loader.dataset)))
    args.logger.info("==========> unlabel_loader length:{}".format(len(unlabl_loader.dataset)))
    ...
    # step 2: 训练模型
    ICT_MedSeg(model, ema_model, label_loader, unlabl_loader, test_loader, args)


def ICT_MedSeg(model, ema_model, label_loader, unlabl_loader, test_loader, args):
    ...
    for epoch in range(max_epoch):
        train_loss = 0
        for i, (img_unlabeled, _) in enumerate(tqdm(unlabl_loader)):
            cur_itrs += 1
            try:
